...
export class AIAdvancedService {
  ...
  async processIntelligentMessage(message: string): Promise<IntelligentResponse> {
    try {
...
`;
    ...
    try {
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'Você é um assistente que analisa intenções de mensagens financeiras em português. Retorne apenas JSON válido.'
          },
          {
            role: 'user',
            content: prompt
  ...
  private fallbackIntentAnalysis(message: string): QueryAnalysis {
    ...
    // Detectar relatório
    const reportKeywords = ['quanto', 'gastei', 'relatório', 'resumo', 'total', 'hoje', 'mês', 'semana'];
  ...
  private async processExpenseIntent(message: string, analysis: QueryAnalysis): Promise<IntelligentResponse> {
  ...
  private async processReportIntent(message: string, analysis: QueryAnalysis): Promise<IntelligentResponse> {
  ...
  private async processQuestionIntent(message: string, analysis: QueryAnalysis): Promise<IntelligentResponse> {
    // Gerar resposta inteligente baseada na pergunta
    const intelligentResponse = await this.generateIntelligentResponse(message, analysis);
  ...
  private async generateIntelligentResponse(message: string, analysis: QueryAnalysis): Promise<string> {
    const prompt = `
...
`;
    ...
    try {
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'Você é o MeuBolso.AI, um assistente financeiro inteligente e amigável.'
          },
          {
            role: 'user',
            content: prompt
      ...
      return completion.choices[0]?.message?.content || 'Vou ajudar você com isso!';
    } catch (error) {
  ...
  private async parseExpense(message: string): Promise<any> {
